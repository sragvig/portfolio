...
2. Upload `index.html`, `style.css`, and `script.js` to the repo (drag-and-drop on github.com works, or `git add . && git commit -m "site" && git push`).
...
- All block/icon art is pure CSS (no image files, nothing to break or re-host).
